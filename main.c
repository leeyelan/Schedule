#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "schedule.h"
#include "linkedList.h"

/* run this program using the console pauser or add your own getch, system("pause") or input loop */

int main(int argc, char *argv[]) {
	
	int exit_flag = 0;        // For running variable
	FILE *fp;                 // FILE pointer
	
	char name[200];           // schedule's name
	char place[100], typeName[100];	// schedule's place and its type
	size_t size;    
	int month;                // schedule's month
	int day;                  // schedule's day
	int type;                 //
	void *list;               // the pointer to point schedule list
	void *ndPtr;              // the pointer to point node
	void *schedInfo;          // the pointer to point schedule information
	int option;               // the variables to select the menu
	int cnt;
	
	
	fp=fopen("schedule.dat","r");     
	if(fp == NULL)
	{
	return -1;
	}                                   // FILE pointer open & error handling
	
	//initializing the list
	printf("Reading the data files... \n");
	list = (void*)list_genList();
	
	
	
	
	//2. read from the file
	while ( fp != NULL )   //
	{	
		sched_genSchedInfo(name,place,type,month,day);  
	  // generate genSchedInfo structure by genSchedInfo function
		
		list_addTail(schedInfo, list);	
		//put the scheduling info instance generated by genSchedInfo function
	}
	
	
    fclose(fp);        // close the fp 
	
	printf("Read done! %i schedules are read\n", list_len(list));
	
	
	//program starts
	while(exit_flag == 0) 
	{
		
		printf("1. print all the schedules\n");
		printf("2. search for schedules in the month\n");
		printf("3. search for schedules in the place\n");
		printf("4. search for specific type schedule\n");
		printf("5. exit\n\n");                              // show the menu
		
		
		
		printf("select an option : ");
		scanf("%d",&option);            // get option from keyboard 
		
		
		
		switch(option)
		{
			case 1: //print all the schedules
				printf("printing all the schedules in the scheduler.....\n\n\n");
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{
                    int f;
					printf("-------------------------------------------\n");
					for(f=0; f<11; f++);
					{
						printf("%i. Schedule Name : %s(%s)",f);
					}
					//file code here -- print count and each scheduling info element
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				
				
				break;
				
			case 2:
				printf("which month ? : ");
				scanf("%i", &month);
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{    
				    sched_print(ndPtr);
					//print scheduling info elements matching to the month
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				}
				
				break;
				
			case 3:
				printf("which place ? : ");
				scanf("%s", place);
				
				ndPtr = list;
				while (list_isEndNode(ndPtr) == 0)
				{
					sched_print(ndPtr);
					//print scheduling info elements matching to the place
					ndPtr = list_getNextNd(ndPtr); //get the next node from the list
					schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
					
					//fill code this part - end
				}
				
				break;
				
			case 4:
				printf("which type ?\n");
				sched_printTypes();
				printf("your choice : ");
				scanf("%s", typeName);
				
				if (sched_convertType(typeName) == 1)
				// convert the type and check if the type is valid */
				{
					ndPtr = list;
					while (list_isEndNode(ndPtr) == 0)
					{
						sched_print(ndPtr);
						// print scheduling info elements matching to the place
						ndPtr = list_getNextNd(ndPtr); //get the next node from the list
						schedInfo = list_getNdObj(ndPtr); //get the object (scheduling info)
						
						//fill code this part - end
					}
				}
				else
				{
					printf("wrong type name!\n");
				}
				break;
				
			case 5:
				printf("Bye!\n\n");
				exit_flag = 1;
				break;
				
			default:
				printf("wrong command! input again\n");
				break;
		}
		
		
	}
	
	return 0;
}
